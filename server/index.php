...
function generate_access_token_first_time($config, $oauth_details)
{
    ...
    error_log(json_encode($oauth_config), 3, $log_file);
    $response = json_decode(make_curl_request($config['access_token_endpoint'], $oauth_config), true);

    error_log("Debug".json_encode($response),3,$log_file);
...
function main()
{
    ...
    $config = array(
        'client_id' => '7f066223fed7e3268fb739aa17e5d026',
        'client_secret' => '1e054c722b0bc5d8d3f7e34cdaf1f894',
    ...
    if (isset($_GET['code'])) {
        ...
        error_log("----->".json_encode($oauth_details), 3, $log_file);
        //error_log(json_encode($_GET), 3, $log_file);
